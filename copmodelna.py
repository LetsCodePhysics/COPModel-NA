  ...
  with open(file_out, 'w') as convert_file: 
    ...
    for node in big_nodes:
      convert_file.write(WriteBootStrapComparisonTableLine(node,G_1_original.nodes[node]['weight']/n1,G_2_original.nodes[node]['weight']/n2,
                                                           '$'+str(np.round(output[node + ' betweenness 1 mean'],4))+     ' \\pm '+str(np.round(output[node + ' betweenness 1 std'],4))     +'$','$'+str(np.round(output[node + ' betweenness 2 mean'],4))+     ' \\pm '+str(np.round(output[node + ' bet2eenness 2 std'],4))     +'$','$'+str(np.round(output[node + ' betweenness d'],2))     +CohensStar(output[node + ' betweenness d'     ]+'$'),
                                                           '$'+str(np.round(output[node + ' normdegree 1 mean'],4))+      ' \\pm '+str(np.round(output[node + ' normdegree 1 std'],4))      +'$','$'+str(np.round(output[node + ' normdegree 2 mean'],4))+      ' \\pm '+str(np.round(output[node + ' normdegree 2 std'],4))      +'$','$'+str(np.round(output[node + ' normdegree d'],2))      +CohensStar(output[node + ' normdegree d'      ]+'$'),
